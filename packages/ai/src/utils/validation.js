 ...
 */
function normalizeExpectedTypes(typeParam) {
    if (typeof typeParam === "string")
        return [typeParam];
    if (Array.isArray(typeParam)) {
        return typeParam.filter((entry) => typeof entry === "string");
    }
    return [];
}
/**
 ...
 */
function matchesExpectedType(value, expectedTypes) {
...
function tryParseNumberString(value, expectedTypes) {
 ...
 */
function tryParseJsonForTypes(value, expectedTypes) {
    const trimmed = value.trim();
    if (!trimmed)
        return { value, changed: false };
    ...
    const looksJsonLiteral = trimmed === "true" || trimmed === "false" || trimmed === "null" || JSON_NUMBER_PATTERN.test(trimmed);
 ...
 */
function setValueAtPointer(root, pointer, value) {
    if (!pointer)
        return value;
    const segments = decodeJsonPointer(pointer);
    let current = root;
 ...
 */
function coerceArgsFromErrors(args, errors) {
...
export function validateToolArguments(tool, toolCall) {
    ...
    let normalizedArgs = originalArgs;
    let changed = false;

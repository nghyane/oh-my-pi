const TRANSIENT_MESSAGE_PATTERN = /overloaded|rate.?limit|usage.?limit|too many requests|service.?unavailable|server error|internal error|connection.?error|unable to connect|fetch failed/i;
const VALIDATION_MESSAGE_PATTERN = /invalid|validation|bad request|unsupported|schema|missing required|not found|unauthorized|forbidden/i;
/**
...
export function isRetryableError(error) {
    ...
    if (VALIDATION_MESSAGE_PATTERN.test(message))
        return false;
    return TRANSIENT_MESSAGE_PATTERN.test(message);
}
export function extractHttpStatusFromError(error, depth = 0) {

...
function formatDeviceModel(system, release, arch) {
...
function getDeviceModel() {
    ...
    const arch = os.arch();
    if (platform === "darwin")
        return formatDeviceModel("macOS", release, arch);
    if (platform === "win32")
        return formatDeviceModel("Windows", release, arch);
...
export async function getKimiCommonHeaders() {
...
async function requestDeviceAuthorization() {
    ...
    const intervalMs = typeof payload.interval === "number" && payload.interval > 0 ? payload.interval * 1000 : DEFAULT_POLL_INTERVAL_MS;
    return {
        userCode,
        deviceCode,
        verificationUri,
...
function parseTokenPayload(payload, refreshTokenFallback) {
...
async function pollForToken(deviceCode, intervalMs, expiresInMs, signal) {
    ...
    while (Date.now() < deadline) {
        ...
        if (error === "slow_down") {
            ...
            if (retryAfter && retryAfter > waitMs)
                waitMs = retryAfter;
            await abortableSleep(waitMs, signal);
            continue;
        }
 ...
 */
export async function loginKimi(options) {
 ...
 */
export async function refreshKimiToken(refreshToken) {
    const response = await fetch(`${resolveOAuthHost()}/api/oauth/token`, {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",

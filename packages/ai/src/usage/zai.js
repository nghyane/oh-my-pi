...
function normalizeZaiBaseUrl(baseUrl) {
...
function buildUsageWindow(id, label, resetsAt, now) {
    if (!resetsAt)
        return { id, label };
...
function getUsageStatus(usedFraction) {
...
function buildModelUsageUrl(baseUrl, now) {
...
async function fetchZaiUsage(params, ctx) {
    ...
    try {
        ...
        if (response.ok) {
            const modelUsagePayload = (await response.json());

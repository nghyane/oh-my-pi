...
function parseResetTimes(data, nowMs) {
    const timeKeys = ["reset_at", "resetAt", "reset_time", "resetTime"];
    ...
    const secondsKeys = ["reset_in", "resetIn", "ttl", "window"];
...
function formatDurationLabel(duration, timeUnit) {
...
function buildWindow(windowData, nowMs) {
    ...
    if (duration === undefined && !label && !resets.resetsAt && !resets.resetInMs)
        return undefined;
    let durationMs;
    if (duration !== undefined) {
        if (timeUnit.toUpperCase().includes("MINUTE"))
            durationMs = duration * 60_000;
        else if (timeUnit.toUpperCase().includes("HOUR"))
            durationMs = duration * 3_600_000;
        else if (timeUnit.toUpperCase().includes("DAY"))
            durationMs = duration * 86_400_000;
        else if (timeUnit.toUpperCase().includes("SECOND"))
...
function buildUsageRow(data, defaultLabel, nowMs) {
...
function buildUsageAmount(row) {
    const amount = { unit: "unknown" };
    if (row.limit !== undefined)
        amount.limit = row.limit;
    if (row.used !== undefined)
        amount.used = row.used;
    if (row.remaining !== undefined)
        amount.remaining = row.remaining;
...
function buildUsageStatus(amount) {
    if (amount.usedFraction === undefined)
        return "unknown";
    if (amount.usedFraction >= 1)
        return "exhausted";
    if (amount.usedFraction >= 0.9)
        return "warning";
    return "ok";
}
function toUsageLimit(row, provider, index, accountId) {
...
function parseUsagePayload(payload, nowMs) {
    if (!isRecord(payload))
        return null;
    const data = payload;
    const rows = [];
...
function resolveCacheExpiry(report, nowMs) {
    if (!report)
...
async function getCachedReport(cache, key, nowMs) {

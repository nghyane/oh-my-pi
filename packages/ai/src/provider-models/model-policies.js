 ...
 */
export const CLOUDFLARE_FALLBACK_MODEL = {
 ...
 */
export function applyGeneratedModelPolicies(models) {
 ...
 */
export function linkSparkPromotionTargets(models) {
    for (const candidate of models) {
        if (!candidate.id.endsWith("-spark"))
            continue;
        const baseId = candidate.id.slice(0, -"-spark".length);
        const fallback = models.find(model => model.provider === candidate.provider && model.api === candidate.api && model.id === baseId);
        if (!fallback)
            continue;

...
import { appendRawHttpRequestDumpFor400, withHttpStatus } from "../utils/http-inspector";
import { sanitizeSurrogates } from "../utils/sanitize-unicode";
import { convertMessages, convertTools, isThinkingPart, mapStopReasonString, mapToolChoice, retainThoughtSignature, } from "./google-shared";
        ...
        try {
            ...
            for (let attempt = 0;; attempt++) {
                ...
                try {
                    ...
                    throw withHttpStatus(new Error(`Cloud Code Assist API error (${response.status}): ${extractErrorMessage(errorText)}`), response.status);
                }
                catch (error) {
            ...
            const streamResponse = async (activeResponse) => {
                ...
                for await (const chunk of readSseJson(activeResponse.body, options?.signal)) {
            ...
            for (let emptyAttempt = 0; emptyAttempt <= MAX_EMPTY_STREAM_RETRIES; emptyAttempt++) {
                ...
                if (emptyAttempt > 0) {
                    ...
                    if (!currentResponse.ok) {
                        ...
                        throw withHttpStatus(new Error(`Cloud Code Assist API error (${currentResponse.status}): ${retryErrorText}`), currentResponse.status);
                    }
                }
            ...
            output.errorMessage = await appendRawHttpRequestDumpFor400(error instanceof Error ? error.message : JSON.stringify(error), error, rawRequestDump);
            output.duration = Date.now() - startTime;
            if (firstTokenTime)
                output.ttft = firstTokenTime - startTime;

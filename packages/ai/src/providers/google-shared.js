                ...
                if (block.type === "text") {
                    ...
                    if (isGemini3Model(model.id) && !thoughtSignature) {
                        const params = Object.entries(block.arguments ?? {})
                            ...
                            .join("\n");
                        parts.push({
                            text: sanitizeSurrogates(`<call_record tool="${block.name}">
...
function mergeCompatibleEnumSchemas(existing, incoming) {
...
function getAnyOfVariants(schema) {
...
function sanitizeSchemaImpl(value, options) {
    ...
    for (const [key, entry] of Object.entries(obj)) {
        ...
        if (!options.insideProperties && UNSUPPORTED_SCHEMA_FIELDS.has(key))
            continue;
        if (options.stripNullableKeyword && key === "nullable")
            continue;
        if (key === "const") {
            constValue = entry;
            continue;
...
export function sanitizeSchemaForGoogle(value) {
...
export function sanitizeSchemaForCloudCodeAssistClaude(value) {
    return sanitizeSchemaImpl(value, {
        insideProperties: false,
 ...
 */
function mergeObjectCombinerVariants(schema, combiner) {
...
function normalizeSchemaForCloudCodeAssistClaude(value) {
    if (Array.isArray(value)) {
...
let cloudCodeAssistSchemaValidator = null;
function getCloudCodeAssistSchemaValidator() {
 ...
 */
function isValidCloudCodeAssistClaudeSchema(schema) {
    try {
        const result = getCloudCodeAssistSchemaValidator().validateSchema(schema);
 ...
 */
export function prepareSchemaForCloudCodeAssistClaude(value) {
 ...
 */
export function convertTools(tools, model) {
    if (tools.length === 0)
        return undefined;
    /**

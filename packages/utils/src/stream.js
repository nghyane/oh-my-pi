...
function parseJsonlChunkFallbackBytes(bytes, beg = 0, end = bytes.length) {
    const values = [];
    let lineStart = beg;
...
function parseJsonlChunkFallbackString(buffer) {
    const values = [];
    let lineStart = 0;
    for (let i = 0; i < buffer.length; i++) {
        if (buffer.charCodeAt(i) !== LF)
            continue;
